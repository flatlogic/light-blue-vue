...
export interface BackendData {
  ...
  performance: {
    sdk: PerformanceMetric
    integration: PerformanceMetric
  }
  server: Record<number, ServerData>
  revenue: RevenueData[]
  mainChart: number[][][]
}
...
function getRevenueData(): RevenueData[] {
...
function getMainChartData(): number[][][] {
  function generateRandomPicks(minPoint: number, maxPoint: number, picksAmount: number, xMax: number): number[][] {
    let x = 0
    let y = 0
    const result: number[][] = []
    const xStep = 1
    ...
    let localMax = maxValues.shift() ?? maxPoint
    let localMin = 0

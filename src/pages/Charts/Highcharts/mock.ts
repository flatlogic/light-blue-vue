...
const wordCloudLines = wordCloudText.split(/[,. ]+/g)
const wordCloudData = wordCloudLines.reduce<WordCloudItem[]>((arr, word) => {
  const obj = arr.find((item) => item.name === word)
  if (obj) {
    obj.weight += 1
  } else {
    arr.push({
...
function generateVectorData(): [number, number, number, number][] {
  const data: [number, number, number, number][] = []

  for (let x = 5; x < 100; x += 5) {
    for (let y = 5; y < 100; y += 5) {
      const length = Math.round(200 - (x + y))
      const direction = Math.round(((x + y) / 200) * 360)
...
export default {
  ...
  sunburst: {
    ...
    subtitle: {
      text:
        'Source <a href="https://en.wikipedia.org/wiki/List_of_countries_by_population_(United_Nations)">Wikipedia</a>',
      style: {
        color: colors.textColor,
      },
    },
